...
// Versi: 1.0.6 (ANTI-STUCK LOGIN & FORCE CACHE CLEAR)
// ============================================================

// --- FORCE UPDATE SERVICE WORKER (Bypass Cache) ---
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.getRegistrations().then(function(registrations) {
    for(let registration of registrations) {
      registration.update(); // Paksa SW mengambil file terbaru
    }
  });
}
  ...
  loadDB() { 
    try {
      ...
      if (data) {
        ...
        if (parsed.history) State.history = { ...State.history, ...parsed.history };
...
const App = {
  ...
  init() {
    ...
    if (typeof APP_DATA === 'undefined') {
      Utils.showToast("Gagal memuat file data.js. Pastikan nama file sesuai!", "error");
      return;
    }
    ...
    setTimeout(() => {
      ...
      if(splash) {
          ...
          splash.style.display = 'none'; // Paksa sembunyi
      }
      
      if (State.user.name) {
        this.checkStreak();
        const appEl = document.getElementById('app');
        if(appEl) appEl.style.display = 'block'; // Paksa tampil
        this.applySettings();
  ...
  showLogin() {
    ...
    if(loginScreen) {
        ...
        loginScreen.style.display = 'flex'; // Paksa tampil
    }
    const grid = document.getElementById('avatar-grid');
    if(grid) grid.innerHTML = this.avatars.map(a => `<div class="avatar-item ${a===this.selectedAvatar?'selected':''}" onclick="App.selectAvatar('${a}', this)">${a}</div>`).join('');
  },
  ...
  // FIX: JALUR PAKSA LOGIN ANTI-STUCK
  login() {
    try {
        const nameEl = document.getElementById('input-name');
        const clsEl = document.getElementById('input-class');
        ...
        if (!name || !cls) {
            Utils.showToast('Nama dan kelas harus diisi!', 'warning');
            return;
        }
        ...
        // Paksa UI Pindah menggunakan Inline Style (Abaikan Konflik CSS)
        const loginScreen = document.getElementById('login-screen');
        const appScreen = document.getElementById('app');
        
        if(loginScreen) {
            loginScreen.classList.add('hidden');
            loginScreen.style.display = 'none'; // Paksa hilang
        }
        
        if(appScreen) {
            appScreen.classList.add('active');
            appScreen.style.display = 'block'; // Paksa muncul
        }
        ...
        Utils.showToast('Terjadi kesalahan sistem, cek console!', 'error');
    }
  },

  checkStreak() {
    const today = new Date().setHours(0,0,0,0);
    const last = new Date(State.user.lastLogin || new Date()).setHours(0,0,0,0);
  ...
  navigate(pageId) {
    ...
    document.querySelectorAll('.page').forEach(el => {
        ...
        el.style.display = 'none'; // Paksa sembunyi page lain
    });
    
    const targetPage = document.getElementById(`page-${pageId}`);
    if(targetPage) {
        targetPage.classList.add('active');
        targetPage.style.display = 'block'; // Paksa tampil page tujuan
    }
    window.scrollTo(0,0);
  ...
  updateDashboard() {
    ...
    setText('hero-name', u.name); setText('hero-class', u.class);
    setText('hero-avatar', u.avatar); setText('header-avatar', u.avatar);
    setText('sidebar-avatar', u.avatar); setText('sidebar-name', u.name); setText('sidebar-class', u.class);

    setText('dash-poin', u.points); setText('dash-streak', u.streak);
    setText('dash-quiz', s.quizCompleted); setText('dash-topics', s.topicsRead);
    
    setText('stat-level', u.level); setText('stat-xp', u.xp);
    setText('stat-badges', State.badges.length); setText('stat-days', s.daysActive);

    const xpBase = lvObj.minXP; const xpReq = lvObj.maxXP;
    const progress = Math.min(100, ((u.xp - xpBase) / (xpReq - xpBase)) * 100);
    
    ['dash-xp-bar', 'sidebar-xp-bar'].forEach(id => {
      const el = document.getElementById(id);
      if(el) el.style.width = `${progress}%`;
    });
    
    setText('dash-level-name', `Level ${lvObj.level} ${lvObj.emoji}`);
    setText('sidebar-level', `Level ${lvObj.level} ${lvObj.emoji}`);
    setText('dash-xp-info', `${u.xp} / ${xpReq} XP`);
    setText('sidebar-xp', `${u.xp} XP`);

    setText('hero-motivation', APP_DATA.dailyMotivation[new Date().getDay() % APP_DATA.dailyMotivation.length]);
    ...
    if (targetList) {
      ...
      setText('daily-target-text', `${completed} / 3 target selesai`);
    }
...
const Materi = {
  ...
  showSubjects() {
    document.getElementById('materi-view-subjects').style.display = 'block';
    document.getElementById('materi-view-topics').style.display = 'none';
    document.getElementById('materi-view-detail').style.display = 'none';
  ...
  showTopics(subjectId) {
    ...
    if(!subject) return;
    document.getElementById('materi-view-subjects').style.display = 'none';
    document.getElementById('materi-view-topics').style.display = 'block';
  ...
  read(subjectId, topicId) {
    ...
    document.getElementById('materi-view-topics').style.display = 'none';
    document.getElementById('materi-view-detail').style.display = 'block';
...
const Quiz = {
  ...
  correctCount: 0, wrongCount: 0, startTotalTime: 0, 
  ...
  start() {
    ...
    this.currentIndex = 0; this.score = 0; this.correctCount = 0; this.wrongCount = 0;
    this.timeLeft = count * 60; this.startTotalTime = this.timeLeft;
  ...
  check(ans, el) {
    ...
    if (ans === q.answer) {
      el.classList.add('correct'); this.score += q.points; this.correctCount++; AudioSystem.success();
    } else {
      el.classList.add('wrong'); this.wrongCount++;
  ...
  checkEssay() {
    const input = document.getElementById('quiz-essay-ans');
    ...
    if(isCorrect) { this.score += q.points; this.correctCount++; AudioSystem.success(); } 
    else { this.wrongCount++; AudioSystem.error(); }
  ...
  finish() {
    ...
    const timeSpent = this.startTotalTime - Math.max(0, this.timeLeft);
    const mins = Math.floor(timeSpent / 60); const secs = timeSpent % 60;
    document.getElementById('result-time').textContent = mins > 0 ? `${mins}m ${secs}s` : `${secs}s`;
...
const Games = {
  ...
  showMenu() {
    document.getElementById('games-select-view').style.display = 'block';
    document.getElementById('game-memory-view').style.display = 'none';
    document.getElementById('game-math-view').style.display = 'none';
    document.getElementById('game-word-view').style.display = 'none';
    clearInterval(this.mathTimerInterval);
  },
  
  // -- MEMORY GAME --
  memCards: [], firstCard: null, memMoves: 0, memPairs: 0, isFlipping: false, 
  startMemory() {
    document.getElementById('games-select-view').style.display = 'none';
    document.getElementById('game-memory-view').style.display = 'block';
    this.memoryReset();
  },
  memoryReset() {
    this.memMoves = 0; this.memPairs = 0; this.firstCard = null; this.isFlipping = false;
  ...
  memFlip(idx) {
    ...
    if(!this.firstCard) {
      ...
      if(this.firstCard.id === secondId) {
        ...
        this.memPairs++; this.isFlipping = false;
        document.getElementById('mem-pairs').textContent = this.memPairs; AudioSystem.success();
        ...
        setTimeout(() => { el.classList.remove('flipped'); fc.classList.remove('flipped'); this.isFlipping = false; }, 800);
      }
    }
  },
  ...
  startMath() {
    document.getElementById('games-select-view').style.display = 'none';
    document.getElementById('game-math-view').style.display = 'block';
    ...
    inputEl.onkeydown = (e) => { if(e.key === 'Enter') this.mathCheck(); };
    this.nextMath();
  ...
  mathCheck() {
    ...
    if(isNaN(ans)) {
        fb.textContent = '⚠️ Ketik angkanya dulu!'; fb.style.color = 'var(--warning)'; return;
    }
  ...
  startWord() {
    document.getElementById('games-select-view').style.display = 'none';
    document.getElementById('game-word-view').style.display = 'block';
    this.wordScore = 0; this.wordQ = 1; document.getElementById('word-score').textContent = 0;
    
    const inputEl = document.getElementById('word-input');
    inputEl.onkeydown = (e) => { if(e.key === 'Enter') this.wordCheck(); };
    this.nextWord();
  },
  nextWord() {
    if(this.wordQ > 5) {
  ...
  wordCheck() {
    ...
    if(!ans) {
        fb.textContent = '⚠️ Ketik kata yang kamu tebak dulu!'; fb.style.color = 'var(--warning)'; return;
    }
...
const Journal = {
  ...
  tab(view, el) {
    ...
    document.getElementById('journal-tulis-view').style.display = view === 'tulis' ? 'block' : 'none';
    document.getElementById('journal-riwayat-view').style.display = view === 'riwayat' ? 'block' : 'none';
